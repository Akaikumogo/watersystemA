...
export class ReportsController {
  ...
  @ApiOperation({
    ...
    description:
      'Get daily energy and water consumption report for the authenticated user'
  })
  @ApiQuery({
    name: 'date',
    required: false,
  ...
  @ApiOperation({
    ...
    description:
      'Get weekly energy and water consumption report for the authenticated user'
  })
  @ApiQuery({
    name: 'weekStart',
    required: false,
    description: 'Week start date in YYYY-MM-DD format (default: current week)'
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Get monthly energy and water consumption report for the authenticated user'
  })
  @ApiQuery({
    name: 'month',
    required: false,
  ...
  @ApiOperation({
    summary: 'Get yearly energy consumption report',
    description:
      'Get yearly energy and water consumption report for the authenticated user'
  })
  @ApiQuery({
    name: 'year',
    required: false,
    description: 'Year in YYYY format (default: current year)'
  })
  @ApiResponse({
    status: 200,
  ...
  @Get('yearly')
  getYearly(@Req() req: any, @Query('year') year?: string) {
    const userId = req.user?.userId ?? req.user?.id;
    if (!userId) {
      throw new UnauthorizedException('User ID not found in token');
    }
    return this.reportsService.getYearly(userId, year);
  }
}
